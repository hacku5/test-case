...
const views = {
    ...
    async orders() {
        ...
        this.renderTable(
            ['Sipariş ID', 'Müşteri', 'Tarih', 'Tutar', 'Durum', 'İçerik', 'İşlem'],
            res.data.map(o => `
                 <td class="p-4 text-gray-500 font-mono text-xs">#${o.id.substring(0, 8)}</td>
                 <td class="p-4 text-white font-medium">${o.customer.name}</td>
                 <td class="p-4 text-gray-400 text-sm">${new Date(o.created_at).toLocaleDateString()}</td>
                 ...
                 <td class="p-4 text-gray-400 text-sm">${o.items_count ?? 0} Parça</td>
                 <td class="p-4 flex gap-2">
                    <button onclick='modal.orderDetail(${JSON.stringify(o).replace(/'/g, "&#39;")})' class="text-blue-400 hover:text-blue-300 text-sm font-medium transition-colors">Detay</button>
                    ${o.status.value === 'pending' ?
                    `<button onclick="actions.cancelOrder('${o.id}')" class="text-red-400 hover:text-red-300 text-sm font-medium transition-colors">İptal</button>`
                    : ''}
                 </td>
            `)
        );
        ...
        document.getElementById('content-area').innerHTML = `
            <div class="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-700 border-b border-gray-600 text-white text-sm uppercase tracking-wider">
                            ${headers.map(h => `<th class="p-4 font-bold text-left">${h}</th>`).join('')}
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-700">
                        ${rows.length ? rows.map(r => `<tr class="hover:bg-gray-700/50 transition-colors">${r}</tr>`).join('') : `<tr><td colspan="${headers.length}" class="p-8 text-center text-gray-500">Kayıt bulunamadı.</td></tr>`}
                    </tbody>
                </table>
            </div>
        `;
...
const actions = {
    ...
    async approveOrder(id) {
        if (!confirm('Siparişi onaylamak istediğinize emin misiniz?')) return;
        try {
            await api.patch(`/orders/${id}/status`, { status: 'completed' });
            toast.show('Sipariş onaylandı.', 'success');
            router.refresh();
            modal.close();
        } catch (e) {
            toast.show(e.message, 'error');
    ...
    async resetDatabase() {
        if (!confirm('DİKKAT! Tüm veritabanı silinecek ve demo verileri yüklenecek. Onaylıyor musunuz?')) return;

        // Show loading toast
        const toastEl = document.getElementById('toast');
        const msgEl = document.getElementById('toast-message');
        toastEl.classList.remove('translate-y-24');
        toastEl.style.borderColor = '#3b82f633';
        msgEl.textContent = 'Veritabanı sıfırlanıyor, lütfen bekleyin...';

        try {
            await api.post('/database/reset', {});
            toast.show('Sıfırlama tamamlandı. Demo veriler yüklendi.', 'success');
            setTimeout(() => {
    ...
    async runTests() {
        ...
        try {
                ...
                logsHtml = `
                    <div class="mt-6 border-t border-gray-700 pt-6">
                        <h4 class="text-white font-bold mb-4 flex items-center gap-2">
                            <svg class="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
                            İşlem "Lifecycle" Geçmişi
                        </h4>
                        <div class="bg-gray-950 p-4 rounded-xl font-mono text-xs text-gray-400 overflow-x-auto border border-gray-800">
                            ${data.logs.split('\n')
                        .filter(l => l.includes('[Process]'))
                        .map(l => {
                            // Extract message after [Process]
                            const message = l.split('[Process]')[1].trim();

                            if (l.includes('.INFO')) return `<div class="text-blue-400 border-l-2 border-blue-500 pl-2 mb-1 flex items-center gap-2"><span class="text-xs text-gray-600">✓</span> ${message}</div>`;
                            if (l.includes('.WARNING')) return `<div class="text-yellow-400 border-l-2 border-yellow-500 pl-2 mb-1 flex items-center gap-2"><span class="text-xs text-gray-600">⚠</span> ${message}</div>`;
                            if (l.includes('.ERROR')) return `<div class="text-red-400 border-l-2 border-red-500 pl-2 mb-1 flex items-center gap-2"><span class="text-xs text-gray-600">⨯</span> ${message}</div>`;
                            return `<div class="opacity-50 py-0.5">${message}</div>`;
                        }).join('')}
                        </div>
                    </div>
                `;
            ...
            results.innerHTML = `
                <div class="bg-gray-900 border border-gray-700 rounded-xl p-6 font-mono text-sm overflow-x-auto">
                    <h4 class="text-white font-bold mb-4">Test Sonuçları</h4>
                    ${outputHtml}
                    ${logsHtml}
                </div>
            `;
...
const modal = {
        ...
        this.body.innerHTML = `
            <form onsubmit="modal.submitProduct(event)" class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="product_name" required class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                </div>
                <div class="grid grid-cols-2 gap-4">
                     <div>
                        <label class="block text-sm font-medium text-gray-400 mb-1">Fiyat (TL)</label>
                        <input type="number" id="product_price" step="0.01" required min="0.01" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-400 mb-1">Stok</label>
                        <input type="number" name="stock_quantity" id="product_stock" required min="0" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                    </div>
                </div>
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg transition-colors">Kaydet</button>
            </form>
    ...
    async submitProduct(e) {
        e.preventDefault();
        const priceInput = document.getElementById('product_price').value;
        ...
        this.body.innerHTML = `
            <form onsubmit="modal.submitEditProduct(event, '${product.id}')" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                     <div>
                        <label class="block text-sm font-medium text-gray-400 mb-1">Fiyat (TL)</label>
                        <input type="number" id="product_price" step="0.01" value="${product.price_raw ? (product.price_raw / 100).toFixed(2) : ''}" class="w-full bg-gray-900 border border-gray-700 rounded-lg p-2 text-white focus:outline-none focus:border-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-400 mb-1">Stok</label>
    ...
    async submitEditProduct(e, id) {
        ...
        const priceInput = document.getElementById('product_price').value;
        data.price = Math.round(parseFloat(priceInput.replace(',', '.')) * 100);
    ...
    orderDetail(order) {
        this.title.textContent = 'Sipariş Detayı';
        const date = new Date(order.created_at).toLocaleString('tr-TR');

        // Generate items table
        const itemsHtml = order.items.map(item => `
            <tr class="border-b border-gray-700 last:border-0">
                <td class="p-3 text-white">${item.product_name || 'Silinmiş Ürün'}</td>
                <td class="p-3 text-gray-400 text-center">${item.quantity}</td>
                <td class="p-3 text-gray-400 text-right">${item.unit_price}</td>
                <td class="p-3 text-white font-mono text-right">${item.subtotal}</td>
            </tr>
        `).join('');
        ...
        this.body.innerHTML = `
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <p class="text-gray-500">Sipariş ID</p>
                    ...
                    <div>
                        <p class="text-gray-500">Müşteri</p>
                        <p class="text-white">${order.customer.name}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Durum</p>
                         <span class="inline-block px-2 py-1 rounded-full text-xs font-medium bg-${order.status.color === 'warning' ? 'yellow-500/10 text-yellow-500' : (order.status.color === 'success' ? 'green-500/10 text-green-500' : 'red-500/10 text-red-500')}">
                            ${order.status.label}
                        </span>
                    </div>
                </div>
                ...
                <div class="bg-gray-900 rounded-lg overflow-hidden border border-gray-700">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-800">
                            <tr>
                                <th class="p-3 text-left text-gray-400 font-normal">Ürün</th>
                                <th class="p-3 text-center text-gray-400 font-normal">Adet</th>
                                <th class="p-3 text-right text-gray-400 font-normal">Birim</th>
                                <th class="p-3 text-right text-gray-400 font-normal">Tutar</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${itemsHtml}
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-700">
                    <span class="text-gray-400 font-medium">Toplam Tutar</span>
                    <span class="text-2xl font-bold text-white font-mono">${order.total_amount}</span>
                </div>

                ${order.status.value === 'pending' ? `
                <div class="grid grid-cols-2 gap-4 pt-4">
                    <button onclick="actions.approveOrder('${order.id}')" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                        Siparişi Onayla
                    </button>
                    <button onclick="actions.cancelOrder('${order.id}')" class="w-full bg-red-600/10 hover:bg-red-600/20 text-red-500 font-bold py-3 rounded-xl transition-colors border border-red-600/20 flex items-center justify-center gap-2">
